...
def test_delete_vote_non_voted(authorized_client,test_posts):
    res=authorized_client.post("/vote/",json={"post_id":test_posts[0].id,"dir":0})
    assert res.status_code==404
    
def test_vote_post_not_exist(authorized_client,test_posts):
    res=authorized_client.post("/vote/",json={"post_id":2309,"dir":1})
    assert res.status_code==404
    
def test_unauthorized_user_voting_post(client,test_posts):
