...
def create_access_token(data:dict):
    ...
    encoded_jwt= jwt.encode(to_endode,SECRET_KEY,ALGORITHM)
    return encoded_jwt
...
def get_current_user(token:str=Depends(oauth2_scheme),db :Session=Depends(database.get_db)):
    credentials_exception=HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail=f"Could not validate credentials: Token is expired",headers={"WWW-Authenticate":"Bearer"})
